...
# 默认就是L2范数
# 并且torch.norm(X, p = 1)这种写法官方也不知道为啥给改了
...
向量范数的计算就是上面的
矩阵范数是另外一种计算方式
矩阵范数的计算方式是: $|X|_p = \max_{x \neq 0} \frac{|Xx|_p}{|x|_p}$
```python
X_matrix = torch.arange(12.0).reshape(3, 4)  # 矩阵: shape=(3, 4)
torch.linalg.norm(X_matrix, ord = 2)         # 得数5.477225575051661
# 下面这种也是 只不过不用做向量或者矩阵判断了
torch.linalg.matrix_norm(X_matrix, ord = 2)  # 得数同上
# 有趣的是 下面这个会直接将矩阵拉长为一维然后计算
torch.linalg.vector_norm(X_matrix, ord = 1)
```
---
